...
public class UserDaoJdbcImpl implements UserDao {
  ...
  public int insertOne(SignupForm form) throws DataAccessException {
    ...
    sqlForUserInsertOne.append("INSERT INTO users (").append(" password,").append(" user_name,")
        .append(" email,").append(" role,").append(" status,").append(" created_at,")
        .append(" updated_at").append(" ) VALUES (").append(" :password,").append(" :user_name,")
        .append(" :email,").append(" :role,").append(" :status,").append(" :created_at,")
        .append(" :updated_at").append(" ) ");

    SqlParameterSource paramForUserInsertOne =
        new MapSqlParameterSource().addValue("password", passwordEncoder.encode(form.getPassword()))
            .addValue("user_name", form.getUserName()).addValue("email", form.getEmail())
            .addValue("role", UserRole.ROLE_GENERAL.toString()).addValue("status", "ALIVE")
