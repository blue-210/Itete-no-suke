...
public class NoteDaoNamedJdbcImpl implements NoteDao {
	...
	private void insertNotesImages(Map<Integer, Integer> imagesIdandSeqMap, Integer noteId) {
		...
		String sqlForNotesImages = "INSERT INTO notes_images ( fk_note_id, fk_images_id, images_seq) VALUES (:fk_note_id, :fk_images_id, :images_seq)";
		
		// パラメータ設定
		imagesIdandSeqMap.entrySet().stream().forEach(entry -> {
			SqlParameterSource paramForNotesImages = new MapSqlParameterSource()
				.addValue("fk_note_id", noteId)
				.addValue("fk_images_id", entry.getKey())
				.addValue("images_seq", entry.getValue());
	...
	private Map<Integer, Integer> insertImages(NoteForm noteForm, Integer noteId) {
		...
		// 画像マスタテーブル登録用SQL
		String sqlForImages = "INSERT INTO images ("
				+ " images_path,"
				+ " created_at,"
				+ " updated_at"
				+ " ) VALUES ("
		...
		for( Image image : noteForm.getImageList() ) {
			SqlParameterSource paramForImages = new MapSqlParameterSource()
					.addValue("images_path", image.getImagePath())
					.addValue("created_at", new Timestamp(new Date().getTime()))
					.addValue("updated_at", new Timestamp(new Date().getTime()));
			
			mapImagesIdandSeq.put(jdbc.queryForObject(sqlForImages, paramForImages, Integer.class), image.getImageSeq());
		}
	...
	private void insertNotesBodyParts(Map<Integer, Integer> bodyPartsIdandSeqMap, Integer noteId) {
		...
		String sqlForNotesBodyParts = "INSERT INTO notes_bodyparts (fk_note_id, fk_bodyparts_id, body_parts_seq) VALUES (:fk_note_id, :fk_bodyparts_id, :body_parts_seq)";
		...
		for( Map.Entry<Integer, Integer> bodyPartsEntry : bodyPartsIdandSeqMap.entrySet() ) {
			SqlParameterSource paramForNotesBodyParts = new MapSqlParameterSource()
					.addValue("fk_note_id", noteId)
					.addValue("fk_bodyparts_id", bodyPartsEntry.getKey())
	...
	private Map<Integer, Integer> insertBodyParts(NoteForm noteForm, Integer noteId) {
		...
		// 部位マスタに登録用SQL
		String sqlForBodyParts = "INSERT INTO bodyparts ("
				+ " body_parts_name,"
				+ " created_at,"
				+ " updated_at"
				+ ") VALUES ("
		...
		for( BodyParts parts : noteForm.getBodyPartsList() ) {
			SqlParameterSource paramForBodyParts = new MapSqlParameterSource()
					.addValue("body_parts_name", parts.getBodyPartsName())
					.addValue("created_at", new Timestamp(new Date().getTime()))
					.addValue("updated_at", new Timestamp(new Date().getTime()));
			
			bodyPartsIdandSeqMap.put(jdbc.queryForObject(sqlForBodyParts, paramForBodyParts, Integer.class), parts.getBodyPartsSeq());
		}
	...
	private void insertNotesMedicine( Map<Integer, Integer> medicineIdandSeqMap, Integer noteId) {
		...
		String sqlForNotesMedicine = "INSERT INTO notes_medicine (fk_note_id, fk_medicine_id, medicine_seq) VALUES (:fk_note_id, :fk_medicine_id, :medicine_seq)";
		...
		for( Map.Entry<Integer, Integer> medicineEntry : medicineIdandSeqMap.entrySet() ) {
			SqlParameterSource paramForNotesMedicine = new MapSqlParameterSource()
					.addValue("fk_note_id", noteId)
					.addValue("fk_medicine_id", medicineEntry.getKey())
	...
	private Map<Integer, Integer> insertMedicine(NoteForm noteForm) {
		...
		// 薬マスタに登録用SQL
		String sqlForMedicine = "INSERT INTO medicine ("
				+ " medicine_name,"
				+ " created_at,"
				+ " updated_at"
				+ ") VALUES ("
		...
		for( Medicine medicine : noteForm.getMedicineList() ) {
			SqlParameterSource paramForMedicine = new MapSqlParameterSource()
					.addValue("medicine_name", medicine.getMedicineName())
					.addValue("created_at", new Timestamp(new Date().getTime()))
					.addValue("updated_at", new Timestamp(new Date().getTime()));
			
			medicineIdandSeqMap.put(jdbc.queryForObject(sqlForMedicine, paramForMedicine, Integer.class), medicine.getMedicineSeq());
		}
	...
	private void insertUsersNotes(NoteForm noteForm, Integer noteId) {
		...
		String sqlForUsersNotes = "INSERT INTO users_notes(fk_user_id, fk_note_id) VALUES (:fk_user_id, :fk_note_id)";
		// パラメータ
		SqlParameterSource paramsForUsersNotes = new MapSqlParameterSource()
				.addValue("fk_user_id", noteForm.getUserId())
				.addValue("fk_note_id", noteId);
	...
	private Integer insertNotes(NoteForm noteForm) {
		// 痛み記録登録用SQL
		String sqlForNotes = "INSERT INTO notes ("
				+ " pain_level,"
				+ " memo,"
				+ " created_at,"
				+ " updated_at"
		...
		SqlParameterSource paramsForNotes = new MapSqlParameterSource()
				.addValue("pain_level", noteForm.getPainLevel())
				.addValue("memo", noteForm.getMemo())
				.addValue("created_at", new Timestamp(new Date().getTime()))
				.addValue("updated_at", new Timestamp(new Date().getTime()));
	...
	private Map<String, Object> findNote(SqlParameterSource params) throws DataAccessException {
		String sqlForNote = "SELECT n.*"
				+ " FROM notes n INNER JOIN users_notes un ON n.note_id = un.fk_note_id"
	...
	public List<NoteForm> getNoteList(long userId) throws DataAccessException {
		...
		for(Map<String, Object> map : list) {
			SqlParameterSource paramsForLists = new MapSqlParameterSource()
					.addValue("note_id", (Long)map.get("note_id"))
					.addValue("user_id", userId);
	...
	private void upsertNotesImages(final Optional<Integer> noteId, Map<Integer, Integer> mapImagesIdandSeq) {
		// 痛み記録_画像関連マスタ登録or更新
		...
		sqlForNotesImages.append("INSERT INTO notes_images (fk_note_id, fk_images_id, images_seq) ")
						 ...
						 .append("ON CONFLICT ON CONSTRAINT notes_images_pkey ")
						 .append("DO NOTHING");
		
		mapImagesIdandSeq.entrySet().stream().forEach(entry -> {
			SqlParameterSource paramForNotesImages = new MapSqlParameterSource()
					.addValue("fk_note_id", noteId.get())
					.addValue("fk_images_id", entry.getKey())
					.addValue("images_seq", entry.getValue());
	...
	private Map<Integer, Integer> upsertImages(NoteForm noteForm) {
		...
		sqlForImagesInsert.append("INSERT INTO images ( ")
					.append("images_path,")
					.append("created_at, ")
					.append("updated_at ")
					.append(") VALUES (")
		...
		sqlForImagesUpdate.append("UPDATE images SET (")
						  .append("images_path = :images_path, ")
						  .append("updated_at = :updated_at ")
						  .append("WHERE images_id = :images_id ")
						  .append("RETURNING images_id ");
		...
		noteForm.getImageList().forEach(image -> {
			if( image.getImageId() != null ) {
				SqlParameterSource paramForImages = new MapSqlParameterSource()
						.addValue("images_id", image.getImageId())
						.addValue("images_path", image.getImagePath())
						.addValue("updated_at", new Timestamp(new Date().getTime()));
				mapImagesIdandSeq.put(jdbc.queryForObject(sqlForImagesUpdate.toString(), paramForImages, Integer.class), image.getImageSeq());
			} else {
				SqlParameterSource paramForImages = new MapSqlParameterSource()
						.addValue("images_path", image.getImagePath())
						.addValue("created_at", new Timestamp(new Date().getTime()))
						.addValue("updated_at", new Timestamp(new Date().getTime()));
				mapImagesIdandSeq.put(jdbc.queryForObject(sqlForImagesInsert.toString(), paramForImages, Integer.class), image.getImageSeq());
			}
		});
		return mapImagesIdandSeq;
	}

	private void upsertNotesBodyParts(final Optional<Integer> noteId, Map<Integer, Integer> mapBodyPartsIdandSeq) {
		// 痛み記録_部位関連マスタ登録or更新
		StringBuffer sqlForNotesBodyParts = new StringBuffer();
		sqlForNotesBodyParts.append("INSERT INTO notes_bodyparts (fk_note_id, fk_bodyparts_id, body_parts_seq) ")
					   .append("VALUES (:fk_note_id, :fk_bodyparts_id, :body_parts_seq) ")
					   .append("ON CONFLICT ON CONSTRAINT notes_bodyparts_pkey ")
					   .append("DO NOTHING");
		
		mapBodyPartsIdandSeq.entrySet().stream().forEach( entry -> {
			SqlParameterSource paramForNotesBodyParts = new MapSqlParameterSource()
					.addValue("fk_note_id", noteId.get())
					.addValue("fk_bodyparts_id", entry.getKey())
	...
	private Map<Integer, Integer> upsertBodyParts(NoteForm noteForm) {
		...
		sqlForBodyPartsInsert.append("INSERT INTO bodyparts (")
							.append("body_parts_name,")
							.append("created_at,")
							.append("updated_at")
							.append(") VALUES (")
							.append(":body_parts_name,")
		...
		sqlForBodyPartsUpdate.append("UPDATE bodyparts SET ")
							.append(" body_parts_name = :body_parts_name, ")
							.append(" updated_at = :updated_at ")
							.append("WHERE body_parts_id = :body_parts_id ")
							.append("RETURNING body_parts_id");
		...
		noteForm.getBodyPartsList().stream().forEach(bodyParts -> {
			if( bodyParts.getBodyPartsId() != null) {
				SqlParameterSource paramForBodyParts = new MapSqlParameterSource()
						...
						.addValue("updated_at", new Timestamp(new Date().getTime()));
				mapBodyPartsIdandSeq.put(jdbc.queryForObject(sqlForBodyPartsUpdate.toString(), paramForBodyParts, Integer.class), bodyParts.getBodyPartsSeq());
			} else {
				SqlParameterSource paramForBodyParts = new MapSqlParameterSource()
						.addValue("body_parts_name", bodyParts.getBodyPartsName())
						.addValue("created_at", new Timestamp(new Date().getTime()))
						.addValue("updated_at", new Timestamp(new Date().getTime()));
				mapBodyPartsIdandSeq.put(jdbc.queryForObject(sqlForBodyPartsInsert.toString(), paramForBodyParts, Integer.class), bodyParts.getBodyPartsSeq());
			}
		});
		return mapBodyPartsIdandSeq;
	}

	private void upsertNotesMedicine(final Optional<Integer> noteId, Map<Integer, Integer> medicineIdandSeqMap) {
		// 痛み記録_薬関連マスタを登録or更新
		StringBuffer sqlForNotesMedicine = new StringBuffer();
		sqlForNotesMedicine.append("INSERT INTO notes_medicine (fk_note_id, fk_medicine_id, medicine_seq) ")
						   .append("VALUES (:fk_note_id, :fk_medicine_id, :medicine_seq) ")
						   .append("ON CONFLICT ON CONSTRAINT notes_medicine_pkey ")
						   .append("DO NOTHING");
		...
		medicineIdandSeqMap.entrySet().stream().forEach( (entry) -> {
			SqlParameterSource paramForNotesMedicine = new MapSqlParameterSource()
				.addValue("fk_note_id", noteId.get())
				.addValue("fk_medicine_id", entry.getKey())
	...
	private Map<Integer, Integer> upsertMedicine(NoteForm noteForm) {
		...
		sqlForMedicineUpdate.append("UPDATE medicine SET ")
							.append("medicine_name = :medicine_name, ")
							.append("updated_at = :updated_at ")
							.append("WHERE medicine_id = :medicine_id ")
							.append("RETURNING medicine_id");
		
		StringBuffer sqlForMedicineInsert = new StringBuffer();
		sqlForMedicineInsert.append("INSERT INTO medicine ( ")
		...
		noteForm.getMedicineList().stream().forEach(medicine -> {
			if( medicine.getMedicineId() != null ) {
				// update
				SqlParameterSource paramsForMedicine = new MapSqlParameterSource()
						.addValue("medicine_name", medicine.getMedicineName())
						.addValue("updated_at", new Timestamp(new Date().getTime()))
						.addValue("medicine_id", medicine.getMedicineId());
				medicineIdandSeqMap.put(jdbc.queryForObject(sqlForMedicineUpdate.toString(), paramsForMedicine, Integer.class), medicine.getMedicineSeq());
			} else {
				// insert
				SqlParameterSource paramsForMedicine = new MapSqlParameterSource()
						.addValue("medicine_name", medicine.getMedicineName())
						.addValue("created_at", new Timestamp(new Date().getTime()))
						.addValue("updated_at", new Timestamp(new Date().getTime()));
				medicineIdandSeqMap.put(jdbc.queryForObject(sqlForMedicineInsert.toString(), paramsForMedicine, Integer.class), medicine.getMedicineSeq());
			}
		});
	...
	private void upsertUsersNotes(NoteForm noteForm, Optional<Integer> noteId) {
		...
		sqlForUsersNotes.append("INSERT INTO users_notes (fk_user_id, fk_note_id) ")
						...
						.append("ON CONFLICT ON CONSTRAINT users_notes_pkey ")
						.append("DO NOTHING");
	...
	private Optional<Integer> updateNote(NoteForm noteForm) {
		...
		sqlForNotes.append("UPDATE notes SET ")
				   .append("pain_level = :pain_level, ")
				   .append("memo = :memo, ")
				   .append("created_at = :created_at, ")
				   .append("updated_at = :updated_at ")
				   .append("WHERE note_id = :note_id ")
		...
		Optional<Integer> noteId = Optional.ofNullable(jdbc.queryForObject(sqlForNotes.toString(), paramsForNotes, Integer.class));
		return noteId;
	}
